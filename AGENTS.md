...
## Expected crib uncertainty sidecar (vendored)

- `src/game/expectedCribPointsUncertainty.json` is generated by the sibling
  `simulate-cribbage-games` repository and published to the **same** rolling
  `expected-crib-points` release as the crib means. Its contract is that
  repository's `docs/uncertainty-sidecars.md`; read it before changing
  anything that consumes the file.
- `npm run table:update:crib` refreshes the pair, and `npm run table:update`
  refreshes it alongside the play table. Never hand-edit either file.
- **The two crib files are a matched pair.** The sidecar carries
  ...
  `npm run test:vendored-tables` re-checks the vendored pair on every commit
  and in CI. That guard is load-bearing rather than defensive: a mismatched
  pair still parses and still renders, so nothing else in the build can
  notice it.
- It is stored minified on one line, like both means tables, rather than as
  the published pretty-printed form. `JSON.stringify(JSON.parse(body))`
  round-trips IEEE 754 doubles exactly, so the reformatting changes no value —
  and the published form is 45,607 lines, which the pull request size gate
  counts in full.
...
- The sidecar loads only after the first recommendation is on screen. It is
  its own build chunk (1.2 MB raw) against the 2.7 MB of means the first
  recommendation already needs, and nothing it carries changes what that
  recommendation says.
- Uncertainty for a figure combined from several buckets must consume exactly
  ...
  decision the mean made instead of re-deriving it.
- `sum(coefficient * se)` is a dependence bound within the reported
  ...
  threshold. Do not let a display of it grow into one; the calibrated
  per-comparison threshold #774 needs is gated on
  `simulate-cribbage-games#135`.
