...
- `src/game/expectedCribPointsUncertainty.json` and
  `src/game/expectedPlayPointsUncertainty.json` are generated by the sibling
  `simulate-cribbage-games` repository and published to the **same** rolling
  releases as the means they describe. Their contract is that repository's
  `docs/uncertainty-sidecars.md`; read it before changing anything that
  consumes either file.
- `npm run table:update:crib` and `npm run table:update:play` each refresh
  one means-and-sidecar pair, and `npm run table:update` refreshes both.
  Never hand-edit any of the four files.
- **Each means file and its sidecar are a matched pair.** The sidecar carries
  `means_sha256`, naming the exact means bytes it was exported against, and
  the contract forbids combining a new means file with an old sidecar. The
  updater refuses to write either file when the digests disagree, and
  `npm run test:vendored-tables` re-checks both vendored pairs on every
  commit and in CI. That guard is load-bearing rather than defensive: a
  mismatched pair still parses and still renders, so nothing else in the
  build can notice it. For the same reason a means file is written from the
  **exact downloaded bytes**, never a newline-normalized copy — the digest
  checked has to be the digest of what is written.
- Both are stored minified on one line, like both means tables, rather than
  as the published pretty-printed form. `JSON.stringify(JSON.parse(body))`
  round-trips IEEE 754 doubles exactly, so the reformatting changes no value —
  and the published forms are 45,607 and 16,425 lines, which the pull request
  size gate counts in full.
- **A missing or rejected sidecar is an unavailable capability, never a
  measured zero**, and a measured standard error of exactly 0 is a real
  value. Guard both with explicit `!== null` checks rather than truthiness,
  the same rule the means loaders already carry.
...
- **One schema describes both documents, so one reader reads both.**
  ...
  and `playUncertainty.ts` carry only what version 1 pins per table — the
  `table` and `n_semantics` values, the four canonical vocabulary lists,
  which of those an identity spells, the weight fields, and the
  record-support predicate. Do not answer a third table, or a schema
  revision, by copying a reader: jscpd at 0% rejects it and the duplication
  would be real.
- **The two documents differ in ways that make a crib-shaped reader reject a
  legal play sidecar.** Play's `ranks` is published **empty** and its
  identity is three parts (`key/role/delta`) where crib's is four
  (`key/role/rank/slot`); its `n_semantics` is `simulation_count` rather than
  `sum_weights`; and it carries no `sum_w2`, so crib's weighted-variance
  denominator has nothing to read and no rule stands in for it. A reader that
  requires every declared list to be non-empty refuses the play document the
  contract specifies. Compare each list against the canonical one instead:
  an empty canonical list then **requires** an empty declared list, which is
  strictly stronger than a non-empty check and still rejects crib's empty
  lists.
- **Derive a canonical key list from the contract, never from the vendored
  means table.** The play sidecar's 1,820 keys are in rank order
  (`A_A_A_A, A_A_A_2, …`) and the vendored `expectedPlayPointsTable.json`
  holds the same 1,820 keys in a different order (`2_2_2_2, 2_2_2_3, …`).
  Identical as sets, different as sequences, so a derivation taken from the
  table rejects every published sidecar. Derive over `A23456789TJQK` and tie
  the two together as **sorted sets** — `playUncertainty.test.ts` does it in
  the browser reader and `vendoredTables.test.mjs` does it across both
  on-disk artifacts, which is a tie the runtime reader cannot make because
  Vite hands it one parsed document rather than both files.
- **Crib's figure is a bound; play's is not, and the wording must not be
  shared.** The crib average is a weighted mean over starter ranks, so
  `sum(coefficient * se)` is a dependence bound within the reported
...
- **Play's figure omits the policy, and nothing published says by how much.**
  ...
  same thing.
- Uncertainty for a figure combined from several buckets must consume exactly
  the buckets that figure consumed. Root and relation records are alternative
  representations of one measurement rather than independent observations, so
  a relation-based mean must never fall back to its rank's root record, and
  the two must not be added. `cribStarterRelationWeight` is exported from
  `expectedCribPoints.ts` so the bound reads the same root-versus-relation
  decision the mean made instead of re-deriving it. Play has no such hazard:
...
- **`security/detect-object-injection` fires on a computed member access even
  where the key is provably one of a fixed set.** `contract.vocabulary[field]`
  is rejected with `field` typed as a union derived from an `as const` tuple,
  because the rule reads the syntax rather than the type. The fix is the same
  named-helper idiom this file already prescribes for
  `no-unnecessary-type-assertion` and for indexed access under
  `noUncheckedIndexedAccess`: lift the lookup into a function with an
  explicit return type that reads through `Reflect.get`, which both gates
  accept. A file-scoped disable is prohibited here anyway.
  ...
  const canonicalList = (
    ...
    Reflect.get(contract.vocabulary, field) as readonly string[];
  ```
...
- **`defaultProps` and the destructuring default are not two spellings of one
  thing, and which one operates depends on the call site.** React 19's
  `createElement` still merges `Component.defaultProps`; the JSX runtime
  (`jsx`/`jsxs`) ignores it entirely. Checked directly on React 19.3:
  `createElement(C, {})` yields `{ a: 1 }` where `jsx(C, {})` yields `{}`.
  So under JSX the destructuring default is the only default there is, and
  `defaultProps` exists here to satisfy `plugin:react/all` rather than to do
  anything at runtime. The trap is Jest's 100% branch threshold: a component
  whose specs render it through `createElement` never reaches its own
  destructuring defaults, so each one is an uncovered `default-arg` branch
  that no test can reach without changing how the spec renders.
  `ScoredPossibleKeepDiscard.test.tsx` renders that way and
  `ScoredPossibleKeepDiscardExpandedRow.test.tsx` renders with JSX, so the
  same prop written the same way in both files was dead in one and live in
  the other. When a `default-arg` branch will not cover, read the branch map
  rather than adding tests — `coverage-final.json`'s `branchMap` names the
  type and line — and normalize at the use site (`prop ?? null`) instead of
  defaulting twice.
- **jscpd compares `.ts` against `.ts` and `.tsx` against `.tsx`, never
  across.** Duplication between a component and a plain module is therefore
  invisible until something moves: extracting a hook out of a `.tsx`
  component into its own `.ts` file surfaced a clone against
  `useMistakeQueueClassifications.ts` that had been there all along. Read
  that as the detector finding real duplication late rather than as the
  extraction creating it — the fix is the shared module the two should have
  had (`src/ui-react/expectedTables.ts`), not a smaller extraction.
