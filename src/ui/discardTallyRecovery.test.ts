...
const expectNoRecordsStored = () => {
  expect(readTallyForDisplay().records).toStrictEqual([]);
};

/*
 * What the store does with values it did not write: damaged, foreign, or
 * simply older. Split from discardTally.test.ts's recording tests because
 * the file grew past the line cap, and because these ask a different
 * question.
 */
describe("discard tally recovery", () => {
  ...
  /*
   * A finite, non-negative `at`/`expectedPointsLoss` is required before this
   * ever reaches recency math: a NaN or Infinity poisons every recencyAt
   * recorded afterward, not just this one, since each is derived from the
   * decision before it.
   */
  it.each([
    { at: NaN },
    { at: Infinity },
    { at: -1 },
    { expectedPointsLoss: NaN },
    { expectedPointsLoss: -1 },
  ])("does not record a decision with invalid %j", (overrides) => {
    clearDiscardTally();

    const summary = recordDiscardDecision(decisionOf(overrides));
  ...
  /*
   * A finite, non-negative check rather than a bare number-type check: NaN
   * and Infinity round-trip through JSON.stringify as null (already
   * rejected by that bare type check), but a negative number is a valid,
   * finite JavaScript number and survives the round trip unchanged. Only an
   * explicit range check catches it, and an uncaught one would have
   * poisoned every later record's recencyAt via Math.max, not just this
   * record's own.
   */
  it("drops a stored record with a negative at or expectedPointsLoss", () => {
    storeRaw(
      asJson(
        storedWith({
          records: [
            { ...validRecord, at: -1, handKey: "negative-at" },
            {
              ...validRecord,
              expectedPointsLoss: -1,
              handKey: "negative-loss",
            },
          ],
        }),
      ),
