...
import { HandPoints, handPoints } from "./Scoring";
...
describe("handPoints", () => {
  ...
  describe("total", () => {
    ...
    it("fifteen and a run", () => {
      ...
      expectTotalPoints([c.EIGHT, c.NINE, c.SEVEN], expectedPoints);
    });

    it("fifteen and a pair", () => {
      const expectedPoints = 4;
      expectTotalPoints([c.SEVEN, c.FOUR, c.FOUR], expectedPoints);
    });

    it("pair and two runs", () => {
      const expectedPoints = 8;
      expectTotalPoints([c.JACK, c.JACK, c.KING, c.QUEEN], expectedPoints);
    });

    it("two fifteens, one pair and two runs", () => {
      const expectedPoints = 12;
      expectTotalPoints([c.SIX, c.SEVEN, c.EIGHT, c.EIGHT], expectedPoints);
    });

    it("four fifteens and two pairs", () => {
      const expectedPoints = 12;
      expectTotalPoints([c.TEN, c.TEN, c.FIVE, c.FIVE], expectedPoints);
    });

    it("two fifteens and a pair", () => {
      const expectedPoints = 6;
      expectTotalPoints([c.QUEEN, c.QUEEN, c.THREE, c.TWO], expectedPoints);
    });
  });
});
