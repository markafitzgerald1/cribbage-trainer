...
export const expectedCribPointBreakdown = (
  ...
  const list = expectedCribPointsByStarterRank(opts);
  const weight = DECK_SIZE - opts.knownCards.length;
  const fifteens = weightedPointValue(list, weight, (item) => item.fifteens);
  const flushes = weightedPointValue(list, weight, (item) => item.flushes);
  const nobs = weightedPointValue(list, weight, (item) => item.nobs);
  const pairs = weightedPointValue(list, weight, (item) => item.pairs);
  const runs = weightedPointValue(list, weight, (item) => item.runs);
