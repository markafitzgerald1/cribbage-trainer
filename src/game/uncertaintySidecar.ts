 ...
 * the pegging figure says it leaves out uncertainty in the pegging strategy,
 * and a document carrying no policy provenance does not substantiate that.
 */
const hasRequiredProvenance = (
  ...
  return (
    ...
    contract.requiredProvenance.every(([field, expected]) => {
      ...
      // A required string may not be empty; upstream's exporter refuses an empty fingerprint.
      return (
 ...
 * rests on it: saying the figure leaves out pegging-strategy uncertainty
 * while the document carries a measured one would hide an available
 * quantity.
