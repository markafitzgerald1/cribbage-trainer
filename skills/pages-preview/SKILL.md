...
- Every eligible open PR auto-publishes to
  ...
  current as the PR head moves and removed when the PR closes
  ...
  Feature-branch CI and preview run on `pull_request` — `opened`,
  `reopened`, and `synchronize`. `push` triggers the workflow only for
  `main` (the production deploy). Opening (or reopening) the PR publishes
  the first preview and runs the first CI directly; `synchronize` handles
  every later push to the PR head. `resolve-preview-pr` reads the PR
  straight from the event payload and rejects a cross-repository or
  Dependabot PR before any deploy step; a fork `pull_request` run also gets
  a read-only token with no secrets, so the publish path is doubly
  unreachable from a fork.
- The push-timing rules these triggers imply bind every PR, not just Pages
  ...
  runs its first CI and publishes its first preview, and pushing a new
  commit to a PR cancels its in-flight run (concurrency, `cancel-in-progress`)
  — so don't push while waiting on a run you need.
